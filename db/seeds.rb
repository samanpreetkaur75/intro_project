...
# Define possible dog sizes
dog_sizes = %w[Small Medium Large]

# Create Breeds and fetch random images
breed_data.each do |breed, sub_breeds|
  if sub_breeds.empty?
    breed_record = Breed.create!(name: breed, size: dog_sizes.sample) # Assign random size
    ...
    sub_breeds.each do |sub_breed|
      breed_record = Breed.create!(name: breed, sub_breed: sub_breed, size: dog_sizes.sample) # Assign random size
